...
150+ HTML element constructors, CSS-in-Python via `Rule`, `Media`, and `Keyframes` with 400+ typed property methods generated from W3C specs, JavaScript helpers for event handling, a component protocol based on `__ztml_render__`, and a built-in server with routing, sessions, SSE, and WebSocket support. HTMX integration is opt-in via pre-configured script tags.
